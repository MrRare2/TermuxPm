...
public class Pm extends BaseCommand {
    ...
    */
    ...
    public void onShowUsage(PrintStream out) {
        ...
        pw.println(
                "Package !stub! manager (package) commands provided by the " + FakeContext.PACKAGE_NAME + " app.\n" +
                "These are similar to commands provided by the Android platform with the /system/bin/pm command. (This is still in development (alpha) + you can still see am related stuff since this is a copy)\n\n" +
                "Usage: pm [subcommand] [options]\n" +
		"       pm path [--user USER_ID] PACKAGE}\n" +
    ...
    private int runStart() throws Exception {
        ...
        if (mUserId == null || mUserId == USER_ALL || mUserId == USER_NULL) {
            System.err.println("Error: Can't start package with user" +
                    " '" + (mUserId != null && mUserId == USER_ALL ? "all" : mUserId) + "'");
            return 1;
        }
